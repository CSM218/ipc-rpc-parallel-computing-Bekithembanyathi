...
public class Master {
    ...
    private void checkWorkerHealth() {
        ...
        for (Map.Entry<String, WorkerInfo> entry : workers.entrySet()) {
            ...
            if (now - info.lastHeartbeat > HEARTBEAT_TIMEOUT) {
                System.out.println("Worker " + entry.getKey() + " timed out! Initiating recovery...");
              
                recoverFromWorkerFailure(entry.getKey(), info);
            }
        }
    }

  
    private void recoverFromWorkerFailure(String workerId, WorkerInfo info) {
        System.out.println("Recovering tasks from failed worker: " + workerId);
        ...
        synchronized (info.assignedTaskIds) {
            for (Integer taskId : info.assignedTaskIds) {
                ...
                if (task != null) {
                    ...
                    System.out.println("Task " + taskId + " reassigned for retry");
                }
            }
        ...
        try { 
            ...
            System.err.println("Error closing socket during recovery: " + e.getMessage());
        }
        
        System.out.println("Recovery complete. Tasks redistributed to available workers.");
    }
    ...
    private static class WorkerInfo {
        ...
        synchronized void sendRPCTaskRequest(Message taskRequest) throws IOException {
    ...
    private class WorkerHandler implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (workerId != null) {
                    System.out.println("Worker disconnected unexpectedly, initiating recovery...");
                    workers.remove(workerId);
                    if (workerInfo != null) {
                        ...
                        synchronized (workerInfo.assignedTaskIds) {
                            for (Integer taskId : workerInfo.assignedTaskIds) {
                                ...
                                if (task != null) {
                                    ...
                                    pendingTasks.offer(task);
                                    System.out.println("Reassigning task " + taskId + " after worker failure");
                                }
                            }
                        }
                    }
        ...
        private void handleRPCTaskResponse(Message message) {
            int taskId = message.getTaskId();
            synchronized (workerInfo.assignedTaskIds) { 
                workerInfo.assignedTaskIds.remove((Integer) taskId); 
            }
            assignedTasks.remove(taskId);
            ByteBuffer cb = ByteBuffer.wrap(message.getPayload()); 
            cb.order(ByteOrder.BIG_ENDIAN);
            int startRow = cb.getInt(); 
            int endRow = cb.getInt(); 
            int startCol = cb.getInt(); 
            int endCol = cb.getInt();
        ...
        private void assignNextTask() throws IOException {
            ...
            if (task != null) {
                ...
                Message taskRequest = new Message(Message.TYPE_TASK, "master", task.taskId, task.data);
                workerInfo.sendRPCTaskRequest(taskRequest);
